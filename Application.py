...
model1 = load_model('devanagari_model_refined.h5')
print(model1)
...
while(cap.isOpened()):
    ...
    blur = cv2.medianBlur(mask, 15)#to get rid of edges
    blur = cv2.GaussianBlur(blur, (5, 5), 0)
    thresh = cv2.threshold(blur, 0 , 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
    cnts = cv2.findContours(thresh.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[1]#contours are nothing but different features.
    center = None
    if len(cnts) >=1:
        ...
        if cv2.contourArea(contour) > 250:
            ...
            pts.appendleft(center) #insert the value of center into the queue
